...
class RegistrationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnRegistration.setOnClickListener {
            ...
            if (isContainEmpty) {
                Toast.makeText(
                    applicationContext,
                    "Harap isi semua field yang tersedia!", Toast.LENGTH_SHORT
                ).show()
            } else if (!isPasswordMatched) {
                Toast.makeText(
                    applicationContext,
                    "Confirm password tidak sesuai!", Toast.LENGTH_SHORT
                ).show()
            } else if (!checkIsUserExist(txtNimReq)){
                Toast.makeText(
                    applicationContext,
                    "Mahasiswa sudah terdaftar!", Toast.LENGTH_SHORT
                ).show()
            } else {
    ...
    private fun checkIsUserExist (
        ...
        return GlobalVariable.isUserAlreadyExist(
            reqTxtNim,
        )
    }
}